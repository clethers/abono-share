...
export async function searchProfiles(supabase, query, excludeIds = []) {
  if (!query || query.length < 2) return [];
  const { data, error } = await supabase
    .from('profiles')
    .select('id, display_name, mobile, email, photo_url')
    .or(`display_name.ilike.%${query}%,mobile.ilike.%${query}%,email.ilike.%${query}%`)
...
export async function createGroup(supabase, name, userId, isPublic = true) {
  // The DB trigger handle_group_creator_member automatically adds the creator as a member.
